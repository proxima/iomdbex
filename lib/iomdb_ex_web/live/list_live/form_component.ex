defmodule IomdbExWeb.ListLive.FormComponent do
  ...
  def update(%{list: list} = assigns, socket) do
    changeset = EquipmentList.change_list(list)
  ...
  def handle_event("validate", %{"list" => list_params}, socket) do
    changeset =
      socket.assigns.list
      |> EquipmentList.change_list(list_params)
  ...
  defp save_list(socket, :edit, list_params) do
    case EquipmentList.update_list(socket.assigns.list, list_params) do
      {:ok, list} ->
  ...
  defp save_list(socket, :new, list_params) do
    case EquipmentList.create_list(list_params) do
      {:ok, list} ->
        notify_parent({:saved, list})
