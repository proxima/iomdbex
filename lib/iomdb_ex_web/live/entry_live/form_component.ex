defmodule IomdbExWeb.EntryLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.simple_form
        ...
        phx-submit="save"
      >
        <.input field={@form[:dicer]} type="text" label="Dicer" />
        <:actions>
  ...
  def update(%{entry: entry} = assigns, socket) do
    changeset = EquipmentList.change_entry(entry)
  ...
  def handle_event("validate", %{"entry" => entry_params}, socket) do
    changeset =
      ...
      |> EquipmentList.change_entry(entry_params)
  ...
  defp save_entry(socket, :edit, entry_params) do
    case EquipmentList.update_entry(socket.assigns.entry, entry_params) do
      {:ok, entry} ->
  ...
  defp save_entry(socket, :new, entry_params) do
    case EquipmentList.create_entry(entry_params) do
      {:ok, entry} ->
        notify_parent({:saved, entry})
